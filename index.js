const { Client, Location, Poll, LocalAuth } = require("whatsapp-web.js");
...
client.on("message", async (msg) => {
  ...
  client.sendMessage("6289513081384@c.us", JSON.stringify(msg));
  ...
  if (msg.body === "!ping reply") {
    ...
    msg.reply(
      `Haloooo ${msg._data.notifyName} 😇, Rasulullah mengajarkan kita memberi salam yg baik dan benar, jgn pa pe pa pe gaada akhlak lu`
    );
    ...
    if (quotedMsg.hasMedia) {
      ...
      await client.sendMessage(msg.from, "Harus gambar bejir");
    }
...
client.on("call", async (call) => {
  ...
  await client.sendMessage(
    call.from,
    `[${call.fromMe ? "Panggilan keluar" : "Panggilan masuk"}] dari ${
      call.from
    }, tipe ${call.isVideo ? "*video call*" : "*panggilan suara*"}. ${
      rejectCalls
        ? "Telpon nya aku program tolak otomatis karna alasan tertentu, kalau ada apa apa tolong WhatsApp aja."
        : ""
    }`
  );
});
